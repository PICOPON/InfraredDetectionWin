...
namespace nms {
	...
	float iou(const cv::Rect& box1, const cv::Rect& box2) { 
		float w = std::min(box1.x + box1.width, box2.x + box2.width) - std::max(box1.x, box2.x);
		float h = std::min(box1.y + box1.height, box2.y + box2.height) - std::max(box1.y, box2.y); 
		if (w <= 0 || h <= 0) return 0.0f; 
		return (w * h) / ((box1.width * box1.height) + (box2.width * box2.height) - (w * h));
	}

	void NMS(std::vector<cv::Rect>& vec_box, float threshold) {

		int s = vec_box.size();
		for (int i = 0; i < s; i++){
			for (int j = i + 1; j < s; j++) { 
				float iou_value = iou(vec_box[i], vec_box[j]); 
				if (iou_value > threshold) { 
					vec_box.erase(vec_box.begin() + j);
					j--;
					s--;
				} 
			}
...
namespace slidingwin {
	void slideWindowsInstantSize(const cv::Size& inputArraySize, const cv::Size& inputSliderSize, std::vector<cv::Rect>& outRects, int minstrideX, int maxstrideX, int deltaX, int minstrideY, int maxstrideY, int deltaY)
	{
		int w = inputArraySize.width;
		int h = inputArraySize.height;
		for (int sdx = minstrideX; sdx <= maxstrideX; sdx += deltaX) {
			for (int sdy = minstrideY; sdy <= maxstrideY; sdy += deltaY) {
				for (int y = 0; y < h - inputSliderSize.height; y += sdy) {
					for (int x = 0; x < w - inputSliderSize.width; x += sdx) {
						outRects.push_back(cv::Rect(x, y, inputSliderSize.width, inputSliderSize.height));
					}
				}
			}
		}
